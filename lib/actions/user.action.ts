...
export async function getUserAnswers(params:GetUserStatsParams){
  try{
    ...
    const userAnswers=await Answer.find({author:userId}).sort({views:-1,upvotes:-1}).skip(skipAmount).limit(pageSize).populate('question',"_id title").populate('author',"_id clerkId name picture")
